...
<x-store.layout :title="$pageTitle">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12"
        ...
        <div class="flex flex-col">
          ...
          <div class="hidden md:block">
            <button class="w-full bg-brand-dark hover:bg-brand-accent text-white font-bold text-sm tracking-widest py-4 transition-colors uppercase disabled:opacity-40 disabled:cursor-not-allowed"
                    :disabled="!activeSize || !inStock">
              Pridať do košíka
            </button>
          </div>
        </div>
  ...
  <div id="sticky-bar" class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 px-3 py-3 flex items-center gap-3 translate-y-full transition-transform duration-300"
       x-data
       x-bind:class="{ 'opacity-50 pointer-events-none': !$root.activeSize || !$root.inStock }">
    <div class="flex flex-1 items-stretch h-13 min-h-[3.25rem]">
      <div class="flex items-stretch border border-gray-300 border-r-0 shrink-0">
        <button type="button" class="w-10 flex items-center justify-center text-lg font-light text-brand-dark select-none px-1"
                @click="$root.qty = Math.max(1, $root.qty - 1)">&minus;</button>
        <div class="w-10 flex items-center justify-center text-sm font-semibold border-x border-gray-300" x-text="$root.qty ?? 1"></div>
        <button type="button" class="w-10 flex items-center justify-center text-lg font-light text-brand-dark select-none px-1"
                @click="$root.qty++">+</button>
      </div>
