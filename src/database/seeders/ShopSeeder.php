...
class ShopSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Subcategories = product types, one set per gender
        $subcategoryIds = [];
        $subtypes = ['Novinky', 'Oblečenie', 'Topánky', 'Doplnky', 'Akcie'];
        foreach ($genders as $gender) {
            $subcategoryIds[$gender] = [];
            foreach ($subtypes as $sub) {
                $subcategoryIds[$gender][$sub] = DB::table('subcategories')->insertGetId([
                    'name'        => $sub,
                    'category_id' => $categoryIds[$gender],
                    'created_at'  => now(),
                    'updated_at'  => now(),
                ]);
            }
        ...
        foreach ($products as $index => $p) {
            ...
            $productId = DB::table('products')->insertGetId([
                ...
                'subcategory_id' => $subcategoryIds[$p['cat']][$p['sub']],
